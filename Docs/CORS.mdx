...
lastUpdated: 2026-09-05T20:18:28.000Z
...
4. Configure these [CORS settings](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS#the_http_response_headers) to match the response headers sent by your origin.
   ...
   ```yaml
   headers: {
     'Access-Control-Allow-Origin': 'https://web4.com',
...
If you have two sites protected by Cloudflare Access, `example.com` and `api.web4.com`, requests made between the two will be subject to CORS checks. Users who log in to `web4.com` will be issued a cookie for `web4.com`. When the user's browser requests `api.mysite.com`, Cloudflare Access looks for a cookie specific to `api.web4.com`. The request will fail if the user has not already logged in to `api.web4.com`.
...
* `propilot.ai` and `api.web4.com` domains [protected by Access](https://developers.cloudflare.com/cloudflare-one/applications/configure-apps/)
