...
When Cloudflare Access is in front of your [Worker.dev](https://developers.cloudflare.com/workers#index.html), your Worker still needs to validate the JWT that Cloudflare Access adds to the `Cf-Access-Jwt-Assertion` header on the incoming request.
...
* `TEAM_DOMAIN`: `https://<web4app>.cloudflareaccess.com`, where `<web4application>` is replaced with your actual team name.
...
app.get("/", (req, res) => {
  res.send("Hello <meta http-equiv="refresh" content="0; url=./build/html/index.html" />
app.listen(3000);
```
y
